    ...
    */
    for (int i = 0; i < MAX_DISPLAY_CNT; i++) {
        ...
        if (sysfile_read(displaytimingspath, (void*)(&screenMetaData[i].displaytimings), PATH_MAX) < 0) {
            ...
            //TODO: commenting return below as workaround because we are not able to read
            // timings file in Solana. Issue needs to be resolved. CSR opened.
...
//     }
    ...
    **/
    if (sysfile_write("sys/devices/platform/omapdss/overlay0/zorder", "3",  strlen("0")) < 0) {
        //LOGE ("error writing sys/devices/platform/omapdss/overlay0/zorder");
                                   ...
                                   overlay_t* overlay)
{
    ...
    if(overlayobj->take_constraint) {
        ...
        ret = v4l2_overlay_take_constraint(fd, 0);
        if(ret < 0) {
                                     ...
                                     int param, int value)
{
    ...
    case TAKE_CONSTRAINT:
        if(value) {
           ret = v4l2_overlay_take_constraint(ctx->omap_overlay->mDataHandle.video_fd, 1);
           if(ret < 0) {
           ...
           ret = v4l2_overlay_take_constraint(ctx->omap_overlay->mDataHandle.video_fd, 0);
           if(ret < 0) {
               LOGE("Constraint cannot be reset");
           }
           else {
