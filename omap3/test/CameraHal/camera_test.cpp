...
#define KEY_MOT_LEDFLASH "mot-led-flash"
#define KEY_MOT_LEDTORCH "mot-led-torch"

#define TESTPATTERN1_ENABLE "enable"
#define TESTPATTERN1_DISABLE "disable"

// Manual Exposure Time in Milliseconds! Default 0 - auto mode
#define KEY_MANUAL_EXPOSURE_TIME_MS "manual-exposure-time-ms"
...
#define MOT_MANUAL_EXPOSURE_MS_MIN 0
#define MOT_MANUAL_EXPOSURE_MS_MAX 100
#define MOT_MANUAL_EXPOSURE_MS_DEFAULT 0
#define MOT_MANUAL_EXPOSURE_MS_STEP_FINE 1
#define MOT_MANUAL_EXPOSURE_MS_STEP_COARSE 10
...
int prevcnt = 0;
int mot_led_flash = 100;
int mot_led_torch = 100;
int mot_manual_exposure_ms = MOT_MANUAL_EXPOSURE_MS_DEFAULT;
...
const char *iso [] = { "auto", "100", "200", "300", "400", "500", "600", "700", 
...
const char *capture[] = { "high-performance", "high-quality", "high-quality-nonzsl", "video-mode" };
...
void initDefaults() {
    ...
    mot_led_flash = 100;
    mot_led_torch = 100;
    mot_manual_exposure_ms = MOT_MANUAL_EXPOSURE_MS_DEFAULT;
    ...
    params.set(KEY_MOT_LEDFLASH, mot_led_flash);
    params.set(KEY_MOT_LEDTORCH, mot_led_torch);
    params.set(KEY_MANUAL_EXPOSURE_TIME_MS, mot_manual_exposure_ms);
...
int menu_motorola() {
    ...
    if (print_menu) {
        printf("\n\n== MOTOROLA MENU ============================\n\n");
        printf("   f. Led Flash Intensity: %d %%\n", mot_led_flash);
        printf("   t. Led Torch Intensity: %d %%\n", mot_led_torch);
        printf("                             \n");
        printf("   e. Manual Exposure Time: %d ms %s\n", mot_manual_exposure_ms,
                      ( 0 == mot_manual_exposure_ms )?"(auto)":"");
        printf("     - r: coarse (+10)\n");
        printf("     - e: fine (+1)\n");
    ...
    switch (ch) {
        ...
        // Manual Exposure: mot_manual_exposure
        case 'r':
            if( mot_manual_exposure_ms >= MOT_MANUAL_EXPOSURE_MS_MAX){
                mot_manual_exposure_ms = MOT_MANUAL_EXPOSURE_MS_MIN;
            } else {
                mot_manual_exposure_ms += MOT_MANUAL_EXPOSURE_MS_STEP_COARSE;
                if( mot_manual_exposure_ms >= MOT_MANUAL_EXPOSURE_MS_MAX){
                    mot_manual_exposure_ms = MOT_MANUAL_EXPOSURE_MS_MAX;
                }
            }
            params.set(KEY_MANUAL_EXPOSURE_TIME_MS, mot_manual_exposure_ms);
        ...
        case 'e':
            if( mot_manual_exposure_ms >= MOT_MANUAL_EXPOSURE_MS_MAX){
                ...
                mot_manual_exposure_ms += MOT_MANUAL_EXPOSURE_MS_STEP_FINE;
            }
            params.set(KEY_MANUAL_EXPOSURE_TIME_MS, mot_manual_exposure_ms);
...
int functional_menu() {
    ...
    case '0':
        ...
        case '$':
            pictureFormat += 1;
            if ( strcmp(params.get(KEY_STEREO_CAMERA), "false") == 0 && pictureFormat > 5 )
                pictureFormat = 0;
    ...
    while ( NULL != cmd && (stopScript == false)) {
        ...
        switch (id) {
            ...
            case 'I':
            {
                ...
                {
                    case 'f':
                        // Script example command: |If10 will set led flash to 10 %
                        mot_led_flash = atoi(cmd + 2);
                        printf("Setting \"%s\" to %d \n", KEY_MOT_LEDFLASH, mot_led_flash);
                        params.set(KEY_MOT_LEDFLASH, mot_led_flash);
                    ...
                    case 't':
                        // Script example command: |It20 will set led torch to 20 %
                        mot_led_torch = atoi(cmd + 2);
                        printf("Setting \"%s\" to %d \n", KEY_MOT_LEDTORCH, mot_led_torch);
                        params.set(KEY_MOT_LEDTORCH, mot_led_torch);
                    ...
                    case 'e':
                        // Script example command: |Ie20 will set Manual Exposure to 20 ms
                        mot_manual_exposure_ms = atoi(cmd + 2);
                        printf("Setting \"%s\" to %d \n",
                                KEY_MANUAL_EXPOSURE_TIME_MS, mot_manual_exposure_ms);
                    ...
                    default:
                        printf("Motorola submenu 'I': Unrecognized subcommand %c !\n", cmd[1]);
                        break;
                }
                break;
            }
