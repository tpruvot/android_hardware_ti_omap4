...
status_t OverlayDisplayAdapter::PostFrame(OverlayDisplayAdapter::DisplayFrame &dispFrame)
{
    ...
    if ( mDisplayState == OverlayDisplayAdapter::DISPLAY_STARTED )
        ...
        if ( mFirstFrame )
            {
            CameraHal::mCameraKPI.stopKPITimer(CAMKPI_Preview);
            mFirstFrame = false;
            }
        else if ( mAfterCapture )
            {
            CameraHal::mCameraKPI.stopKPITimer(CAMKPI_RestartPreview);
            mAfterCapture = false;
            }
        else if (CameraFrame::CameraFrame::SNAPSHOT_FRAME == dispFrame.mType)
            {
            CameraHal::mCameraKPI.stopKPITimer(CAMKPI_Postview);
            }
