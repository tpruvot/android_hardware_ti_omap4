...
void AppCallbackNotifier::notifyEvent()
{
    ...
    switch(msg.command)
        ...
        case AppCallbackNotifier::NOTIFIER_CMD_PROCESS_CAF:

            if (  ( NULL != mCameraHal.get() ) &&
                  ( NULL != mNotifyCb ) /* &&
                  ( mCameraHal->msgTypeEnabled(CAMERA_MSG_CAF) )*/ )
                {
                CAMHAL_LOGEB("sending CAMERA_MSG_CAF to APP %d", (int) msg.arg1);
                mNotifyCb(CAMERA_MSG_CAF, (int) msg.arg1, 0, mCallbackCookie);
                }
...
static void copy2Dto1D(void *dst, void *src, int width, int height, size_t stride, uint32_t offset, unsigned int bytesPerPixel,
    size_t length, const char *pixelFormat)
{
    unsigned int alignedRow, row;
    unsigned char *bufferDst, *bufferSrc, *bufferDstEnd, *bufferSrcEnd;
...
status_t AppCallbackNotifier::stopPreviewCallbacks()
{
    ...
    //IKSTABLEFIBE-12028, wait for notifyFrame frame to exit
